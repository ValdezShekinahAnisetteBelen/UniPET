...
<section class="py-5 bg-light">
    <div class="container px-4 px-lg-5 mt-5">
        ...
        <div class="row gx-4 gx-lg-5 row-cols-2 row-cols-md-3 row-cols-xl-4 justify-content-center">
            <div class="col mb-5" v-for="relatedProduct in relatedProducts" :key="relatedProduct.id">
                <div class="card h-100">
                    ...
                    <div class="card-footer p-4 pt-0 border-top-0 bg-transparent">
                        <div class="text-center">
                            <router-link :to="'/ProductDetails/' + encodeURIComponent(relatedProduct.name)" :title="relatedProduct.name">
                                {{ relatedProduct.name }} View Options
                            </router-link>
                        </div>
                    </div>
                </div>
...
const $ = jQuery.noConflict();
  ...
  computed: {
    relatedProducts() {
        return this.products.filter(p => p.productgroup === this.product.productgroup);
    }
},
  methods: {
  ...
  try {
    ...
    try {
        ...
        this.product.image = '/' + this.product.image.replace(/\\/g, '/');
    } catch (error) {
