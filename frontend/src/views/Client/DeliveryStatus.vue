...
<template>
    ...
    <img :src="currentGif" alt="Delivery Truck GIF" width="200" height="150">
    <div id="message" class="h2 mb-3">{{ currentMessage }}</div>
    <button class="btn btn-success btn-lg" v-show="showButton" @click="openOrderHistory">Open My Purchases</button>
  </div>
              </div>
            <!-- card -->
    ...
    export default {
        data() {
      return {
        currentGif: "User/wp-content/uploads/sites/2/2019/08/preview.gif",
    ...
    mounted() {
      setTimeout(() => {
        this.currentMessage = "Your Order has been submitted...";
        setTimeout(() => {
          this.showButton = true;
        }, 15 * 1000); // Show the button after 15 seconds
      }, 10 * 1000); // Show "Loading..." for 10 seconds
    },
